## Estimating the time of arrival based on fossil/archaeological records
## main principle =  the algorithm use a weighted likelihood approach. The 'weights' depends on the pairwise distance between the targetted location
## (i.e where we want estimate the arrival time) and the surrounding fossil sites.
## The algorithm uses a exponential base with main parameter estimated using biais/variance (IMSE) minimisation technique (double kernel) s
## 
## biological hypothesis of the model = the age of the fossil at the x location follow a uniform distribution between 0 et T(x)= arrival date at x 
##
##

##solow.fct = estimates of arrival date at location (xx,yy)
##        outputs: Test = estimated date
##                biais = estimated bias
##		             sd = standard deviation around Test
##
...
## 
##nbcoeur=number of core reuqested for calculation 
# 
# 
