program Simulated_annealing_path(Mexico_Brazil)
...
rs=80;cs=299;re=94;ce=340;%startpt=max(max(mat.time));[rs,cs]=find(mat.time==startpt);[re,ce]=find(mat.time==endpt);endpt=min(min(mat.time));
...
save -ascii Trajectory_Mexico-Brazil_(path#1).txt path;save -ascii Trajectory_Mexico-Brazil_(enthropy#1).txt MHent;
save -ascii Trajectory_Mexico-Brazil_(Temp#1).txt TMHent;save -ascii Trajectory_Mexico-Brazil_(Finalproba#1).txt pfinal;
...
rs_2=116;cs_2=331;re_2=94;ce_2=340;[path_2, mat_2, ~]=shortpath(re_2,ce_2,rs_2,cs_2,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
...
%% calcul de l'entropie de base  => somme des proba entre chaque cellule de grille
np=length(path(:,1));p=proba3(path);inipath=path;Entropie=sum(p);clear p;
stop=0;TMH=20;MHrate=3.7;MHent=[];TMHent=[];stoptime=20000;cpt=1;%0.12 est pas mal
figure(3),plot(cpt,Entropie,'ok','Markersize',4)%,hold on,plot(cpt,stop,'*b','Markersize',4);
figure(4),plot(cpt,TMH,'*b','Markersize',4);
...
save -ascii Trajectory_Mexico-Brazil_(path#2).txt path;save -ascii Trajectory_Mexico-Brazil_(enthropy#2).txt MHent;
save -ascii Trajectory_Mexico-Brazil_(Temp#2).txt TMHent;save -ascii Trajectory_Mexico-Brazil_(Finalproba#2).txt pfinal;
...
rs_7=143;cs_7=310;re_7=133;ce_7=316;[path_7, mat_7, ~]=shortpath(re_7,ce_7,rs_7,cs_7,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
rs_8=133;cs_8=316;re_8=94;ce_8=340;[path_8, mat_8, ~]=shortpath(re_8,ce_8,rs_8,cs_8,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  

path=[path_1;path_2;path_3;path_4;path_5;path_6;path_7;path_8];path(1:length(path(:,1)))=(1:length(path(:,1)))';mat.flag=mat_1.flag + mat_2.flag + mat_3.flag + mat_4.flag + mat_5.flag + mat_6.flag + mat_7.flag + mat_8.flag;[row,col]=find(mat.flag>1);mat.flag(row(:),col(:))=1;
coast = load('coast');seaclr=brewermap(15,'RdBu');seaclr1=seaclr(9,:);
figure('Color','w'),ax=worldmap({'North America','South America'});gridm('on');setm(gca,'ParallelLabel','off','MeridianLabel','off');
                    geoshow(coast.lat,coast.long,'DisplayType','polygon','Facecolor','w'),
                    geoshow(flipud(coast.lat),flipud(coast.long),'DisplayType','polygon','FaceColor',seaclr1),
                    hold on,surfm(yg+0.5,xg2-0.5,mat.time);hold on,plotm(mat.lat(rs_8,cs_8),mat.lon(rs_8,cs_8),'dm','Markersize',6,'Markerfacecolor','m','Markeredgecolor',[0 0 0]);
                    hold on,plotm(mat.lat(re_1,ce_1),mat.lon(re_1,ce_1),'pm','Markersize',6,'Markerfacecolor','m','Markeredgecolor',[0 0 0]);
                    hold on,plotm(path(:,4),path(:,5),'-^m','Markersize',6,'Markerfacecolor','m','Markeredgecolor',[0 0 0]);
...
save -ascii Trajectory_Mexico-Brazil_(path#3).txt path;save -ascii Trajectory_Mexico-Brazil_(enthropy#3).txt MHent;
save -ascii Trajectory_Mexico-Brazil_(Temp#3).txt TMHent;save -ascii Trajectory_Mexico-Brazil_(Finalproba#3).txt pfinal;
...
mat.time=load('maskedHumantimingraster_v2.txt');mat.time(35:36,25:35)=NaN;[l,c]=size(mat.time);mat.flag=zeros(l,c);%st=50;mat.time(:,st:end)=NaN;mat.time(17:20,34:38)=NaN;mat.flag=zeros(l,c);
tr=160;xg=-179.5:1:179.5;yg=-89.5:89.5;xg2=[xg(:,tr:end),xg(:,1:tr-1)];yg=flipud(yg');mat.lat=repmat(yg,1,c);mat.lon=repmat(xg2,l,1);
rs_1=80;cs_1=299;re_1=106;ce_1=309;[path_1, mat_1, ~]=shortpath(re_1,ce_1,rs_1,cs_1,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
rs_2=106;cs_2=309;re_2=106;ce_2=324;[path_2, mat_2, ~]=shortpath(re_2,ce_2,rs_2,cs_2,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
rs_3=106;cs_3=324;re_3=94;ce_3=340;[path_3, mat_3, ~]=shortpath(re_3,ce_3,rs_3,cs_3,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
...
save -ascii Trajectory_Mexico-Brazil_(path#4).txt path;save -ascii Trajectory_Mexico-Brazil_(enthropy#4).txt MHent;
save -ascii Trajectory_Mexico-Brazil_(Temp#4).txt TMHent;save -ascii Trajectory_Mexico-Brazil_(Finalproba#4).txt pfinal;
...
mat.time=load('maskedHumantimingraster_v2.txt');mat.time(35:36,25:35)=NaN;[l,c]=size(mat.time);mat.flag=zeros(l,c);%st=50;mat.time(:,st:end)=NaN;mat.time(17:20,34:38)=NaN;mat.flag=zeros(l,c);
tr=160;xg=-179.5:1:179.5;yg=-89.5:89.5;xg2=[xg(:,tr:end),xg(:,1:tr-1)];yg=flipud(yg');mat.lat=repmat(yg,1,c);mat.lon=repmat(xg2,l,1);
rs_1=80;cs_1=299;re_1=98;ce_1=305;[path_1, mat_1, ~]=shortpath(re_1,ce_1,rs_1,cs_1,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
rs_2=98;cs_2=305;re_2=116;ce_2=320;[path_2, mat_2, ~]=shortpath(re_2,ce_2,rs_2,cs_2,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
rs_3=116;cs_3=320;re_3=94;ce_3=340;[path_3, mat_3, ~]=shortpath(re_3,ce_3,rs_3,cs_3,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
...
save -ascii Trajectory_Mexico-Brazil_(path#5).txt path;save -ascii Trajectory_Mexico-Brazil_(enthropy#5).txt MHent;
save -ascii Trajectory_Mexico-Brazil_(Temp#5).txt TMHent;save -ascii Trajectory_Mexico-Brazil_(Finalproba#5).txt pfinal;
...
mat.time=load('maskedHumantimingraster_v2.txt');mat.time(35:36,25:35)=NaN;[l,c]=size(mat.time);mat.flag=zeros(l,c);%st=50;mat.time(:,st:end)=NaN;mat.time(17:20,34:38)=NaN;mat.flag=zeros(l,c);
tr=160;xg=-179.5:1:179.5;yg=-89.5:89.5;xg2=[xg(:,tr:end),xg(:,1:tr-1)];yg=flipud(yg');mat.lat=repmat(yg,1,c);mat.lon=repmat(xg2,l,1);
rs_1=80;cs_1=299;re_1=81;ce_1=317;[path_1, mat_1, ~]=shortpath(re_1,ce_1,rs_1,cs_1,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
rs_2=81;cs_2=317;re_2=94;ce_2=340;[path_2, mat_2, ~]=shortpath(re_2,ce_2,rs_2,cs_2,mat);%% calcul de la trajectoire de base  => distance la plus courte entre le point initial et le point final en evitant les point sans dates de colonisation et deja utiliser                  
...
%% calcul de l'entropie de base  => somme des proba entre chaque cellule de grille
np=length(path(:,1));p=proba3(path);inipath=path;Entropie=sum(p);clear p;
stop=0;TMH=20;MHrate=3;MHent=[];TMHent=[];stoptime=20000;cpt=1;%0.12 est pas mal
figure(3),plot(cpt,Entropie,'ok','Markersize',4)%,hold on,plot(cpt,stop,'*b','Markersize',4);
figure(4),plot(cpt,TMH,'*b','Markersize',4);
...
save -ascii Trajectory_Mexico-Brazil_(path#6).txt path;save -ascii Trajectory_Mexico-Brazil_(enthropy#6).txt MHent;
save -ascii Trajectory_Mexico-Brazil_(Temp#6).txt TMHent;save -ascii Trajectory_Mexico-Brazil_(Finalproba#6).txt pfinal;
...
tr=160;xg=-179.5:1:179.5;yg=-89.5:89.5;xg2=[xg(:,tr:end),xg(:,1:tr-1)];yg=flipud(yg');mat.lat=repmat(yg,1,c);mat.lon=repmat(xg2,l,1);
rs=80;cs=299;re=94;ce=340;;[path, mat, ~]=shortpath(re,ce,rs,cs,mat);p=proba3(path);Entropie=sum(p);
save -ascii Short_Trajectory_Mexico-Brazil_(path#1).txt path;save -ascii Short_Trajectory_Mexico-Brazil_(enthropy#1).txt Entropie;save -ascii Short_Trajectory_Mexico-Brazil_(Finalproba#1).txt p;
